...
const ManagementSRP = () => {
    ...
    const filters = useSelector(selectSRPFilters);
    const dateRange = useSelector(selectSRPDateRange);
    const pagination = useSelector(selectSRPPagination);
    const loading = useSelector(selectSRPLoading);
    ...
    useEffect(() => {
        ...
        const pollStatus = () => {
             dispatch(fetchSRPStatus()).unwrap().then(data => {
                ...
                statusPollRef.current = setTimeout(pollStatus, delay > 1000 ? delay : 5000);
             });
        };
        pollStatus();
    ...
    // The component dispatches setFilter/etc, which updates the store.
    // This effect listens to store changes and triggers the fetch.
    useEffect(() => {
        dispatch(fetchSRPData());
    }, [dispatch, dateRange, activeDivisions, pagination.page, pagination.limit, filters]);


    // --- HANDLERS ---
    const handleUpdateCategory = (entryId, newCategory) => {
        dispatch(updateSRPCategory({ entryId, category: newCategory }));
    };
                ...
                <div className="flex gap-4 items-center flex-wrap">
                    <div className="flex items-center gap-2 bg-black/30 p-1.5 rounded border border-white/10">
                        <input 
                            ...
                            onChange={(e) => dispatch(setDateRange({ start: e.target.value }))}
                        ...
                        <input 
                            type="date" 
                            value={dateRange.end}
                            onChange={(e) => dispatch(setDateRange({ end: e.target.value }))}
                            className="bg-transparent text-xs text-white outline-none w-24" 
                        />
                        <button 
                            onClick={() => dispatch(setDateRange({ start: '', end: '' }))}
                    ...
                    <div className="flex gap-2 text-xs">
                        {['1','2','3','4','5','6','7'].map(div => (
                            <label key={div} className="flex items-center gap-1 cursor-pointer select-none">
                                <input 
                                    type="checkbox" 
                                    checked={activeDivisions.includes(div)}
                                    onChange={() => dispatch(toggleDivision(div))}
                    ...
                    <table className="w-full text-left text-xs text-slate-400 border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5 opacity-100 transition-opacity duration-200" style={{ opacity: loading ? 0.5 : 1 }}>
                            {summary?.transactions?.map(tx => (
                                <TransactionRow key={tx.entry_id} tx={tx} onUpdateCategory={handleUpdateCategory} divisionMap={divisionMap} />
                            ))}
